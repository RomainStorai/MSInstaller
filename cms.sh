...
if [ $answer = "Y" ]; then
	...
	generated=`cat /dev/urandom | tr -dc 'a-zA-Z0-9' | fold -w 12 | head -n 1`
	cd /var/www/html
	...
	ended_loop=true
	while $ended_loop; do
		echo "${RED}DO NOT CLOSE THIS SCRIPT! ${BLUE}When you finished following the steps, please enter 'OK' to continue:${WHITE} ('stop' to cancel)"
		read ok
		if [ $ok = "stop" ]; then
		...
		if [ ! -f cms.zip ]; then
			echo "${RED}You have not uploaded your CMS!"
		else
			ended_loop=false
			unzip cms.zip
			rm cms.zip
